...
public class EastMoneyBoardMetricsDataProvider implements TradingMarketDataProvider {
    ...
    public EastMoneyBoardMetricsDataProvider(EastMoneyTradingMarketDataProvider marketDataProvider,
        ...
        this.client = RestClient.builder()
                .defaultHeader("User-Agent", "Mozilla/5.0 personal-assistant")
                .build();
    }

    @Override
    public String name() {
        return marketDataProvider.name();
    }
    ...
    public MarketSnapshot fetch(LocalDate tradeDate) {
        ...
        try {
            ...
            BoardMetrics board = parseBoardMetrics(
                    get(LIMIT_UP_POOL_URL.formatted(date)),
                    get(BROKEN_BOARD_POOL_URL.formatted(date)));
            String rawMetrics = objectMapper.createObjectNode()
                    .put("marketMetrics", market.rawMetrics())
                    .put("limitMethod", "东方财富涨停池")
                    .put("brokenBoardMethod", "东方财富炸板池")
                    .put("brokenBoardRateMethod", "炸板数/(封板数+炸板数)")
                    .put("streakMethod", "涨停池最高连板数")
                    .toString();
            ...
            throw new BusinessException(ErrorCode.INTERNAL_ERROR,
                    "东方财富涨停炸板数据采集失败：" + exception.getMessage());
        }
    }
    ...
    BoardMetrics parseBoardMetrics(JsonNode limitUpResponse, JsonNode brokenBoardResponse) {
        ...
        BigDecimal brokenBoardRate = touchedLimitCount == 0 ? BigDecimal.ZERO
                : BigDecimal.valueOf(brokenBoardCount)
                .multiply(BigDecimal.valueOf(100))
                .divide(BigDecimal.valueOf(touchedLimitCount), 2, RoundingMode.HALF_UP);
    ...
    private JsonNode get(String url) {
        ...
        try {
            ...
            throw new IllegalStateException("涨停炸板响应解析失败", exception);
        }
    }

    record BoardMetrics(Integer limitUpCount, Integer brokenBoardCount,
                        BigDecimal brokenBoardRate, Integer maxStreak) {}
}
