...
public class PublicGoldQuoteService {
    ...
    public GoldPublicQuoteResponse latest() {
        try {
            ...
            return new GoldPublicQuoteResponse(List.of(
                    new GoldPublicQuoteResponse.Quote("XAU_USD", "国际现货黄金", usdPerOunce.setScale(2, RoundingMode.HALF_UP),
                            "USD/盎司", "国际现货黄金 XAU/USD 实时参考价", false),
                    new GoldPublicQuoteResponse.Quote("XAU_CNY_GRAM", "国际金折算人民币", cnyPerGram,
                            "元/克", "按实时 XAU/USD × USD/CNY ÷ 31.1034768 折算，不代表上海金交所或品牌零售价", true)
            ), usdCny.setScale(4, RoundingMode.HALF_UP), quoteTime == null ? fetchedAt : quoteTime, fetchedAt,
                    "Gold API + ExchangeRate-API", 60);
