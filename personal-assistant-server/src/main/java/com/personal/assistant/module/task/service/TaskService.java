...
public class TaskService {
 ...
 private void validateDetail(TaskUpsertRequest r,boolean creating){if(r.itemType()==TaskType.LIFE&&!StringUtils.hasText(r.category()))throw new BusinessException(ErrorCode.VALIDATION_ERROR,"生活分类不能为空");if(r.itemType()==TaskType.WORK&&!StringUtils.hasText(r.workType()))throw new BusinessException(ErrorCode.VALIDATION_ERROR,"工作类型不能为空");if(r.itemType()==TaskType.WORK&&r.deadline()!=null&&creating&&r.deadline().isBefore(LocalDateTime.now()))throw new BusinessException(ErrorCode.VALIDATION_ERROR,"截止时间不能早于创建时间");}
 private void apply(TaskItem i,TaskUpsertRequest r){i.setTitle(r.title().trim());i.setItemType(r.itemType().name());i.setPriority(r.priority().name());i.setStatus(r.status().name());i.setPlanTime(r.planTime());i.setDeadline(r.deadline());i.setReminderEnabled(Boolean.TRUE.equals(r.reminderEnabled()));i.setTags(r.tags());i.setRemark(r.remark());}
 ...
 private TaskResponse toResponse(TaskItem i){LifeItemDetail l=TaskType.LIFE.name().equals(i.getItemType())?lifeMapper.selectById(i.getId()):null;WorkItemDetail w=TaskType.WORK.name().equals(i.getItemType())?workMapper.selectById(i.getId()):null;return new TaskResponse(i.getId(),i.getTitle(),TaskType.valueOf(i.getItemType()),TaskPriority.valueOf(i.getPriority()),TaskStatus.valueOf(i.getStatus()),i.getPlanTime(),i.getDeadline(),Boolean.TRUE.equals(i.getReminderEnabled()),i.getTags(),i.getRemark(),l==null?null:l.getCategory(),w==null?null:w.getWorkType(),w==null?null:w.getProjectName(),i.getStatusChangedAt(),i.getCreatedAt(),i.getUpdatedAt());}
}