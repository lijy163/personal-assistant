    ...
    <el-container class="app-shell">
      ...
      <el-container>
        <el-header class="app-header">
          <div class="title-row"><el-button class="mobile-menu-button" text @click="mobileMenu=true">☰</el-button><div><div class="page-title">{{pageTitle}}</div><div class="page-desc" :title="inspirationEnglish">{{ inspirationText }}</div></div></div>
...
import { getDailyInspiration, globalSearch, type SearchResult } from '@/api/productivity';
...
const inspirationText = ref('今天也把重要的事情放到系统里');
const inspirationEnglish = ref('');
...
onMounted(async () => {
  try {
    const inspiration = (await getDailyInspiration()).data;
    inspirationText.value = inspiration.translation || inspiration.content || inspirationText.value;
    inspirationEnglish.value = inspiration.translation ? inspiration.content : '';
  } catch {
    // 保留默认文案，不影响主布局加载。
  }
});
