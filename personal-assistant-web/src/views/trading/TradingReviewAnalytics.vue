<template>
  <div v-if="analytics" class="analytics">
    <div class="indicator-grid">
      ...
      <el-card><small>预警后交易响应</small><strong>{{ percent(analytics.execution.alertTradeRate) }}</strong><span>{{ analytics.execution.alertLinkedTrades }} / {{ analytics.execution.alertEvents }} 条预警</span></el-card>
      <el-card><small>平均响应时间</small><strong>{{ minutes(analytics.execution.averageAlertResponseMinutes) }}</strong><span>{{ executionStatus(analytics.execution.status) }}</span></el-card>
      <el-card><small>仓位偏差</small><strong>{{ percent(analytics.execution.averagePositionDeviation) }}</strong><span>来自计划备注或计划外交易占比</span></el-card>
      <el-card><small>闭环状态</small><strong>{{ executionStatus(analytics.execution.status) }}</strong><span>提醒、交易、计划是否连起来</span></el-card>
    </div>
    <el-alert v-if="analytics.advancement.status!=='AVAILABLE'" :title="analytics.advancement.status" description="系统会从本版本开始积累每日涨停股票明细，至少两个收盘交易日后自动计算晋级率。" type="info" :closable="false"/>
...
import { getReviewAnalytics, type ReviewAnalytics } from '@/api/tradingReview';
const props = defineProps<{ tradeDate: string; refreshKey?: number }>();
const analytics = ref<ReviewAnalytics>();
async function load() { if (props.tradeDate) analytics.value = (await getReviewAnalytics(props.tradeDate)).data; }
watch(() => [props.tradeDate, props.refreshKey], load);
onMounted(load);
const trendOption = computed<EChartsOption>(() => {
  const rows = analytics.value?.fiveDayTrend || [];
  return { tooltip: { trigger: 'axis' }, legend: { data: ['情绪评分', '涨停数', '跌停数', '炸板率'] }, xAxis: { type: 'category', data: rows.map(v => v.tradeDate.slice(5)) }, yAxis: [{ type: 'value', name: '评分/家数' }, { type: 'value', name: '炸板率', max: 100 }], series: [{ name: '情绪评分', type: 'line', smooth: true, data: rows.map(v => v.sentimentScore) }, { name: '涨停数', type: 'line', data: rows.map(v => v.limitUpCount) }, { name: '跌停数', type: 'line', data: rows.map(v => v.limitDownCount) }, { name: '炸板率', type: 'line', yAxisIndex: 1, data: rows.map(v => v.brokenBoardRate) }] };
});
const timelineOption = computed<EChartsOption>(() => {
  const rows = analytics.value?.intradayTimeline || [];
  return { tooltip: { trigger: 'axis' }, legend: { data: ['情绪评分', '上涨家数', '下跌家数'] }, xAxis: { type: 'category', data: rows.map(v => new Date(v.quoteTime).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', hour12: false })) }, yAxis: [{ type: 'value', name: '评分' }, { type: 'value', name: '家数' }], series: [{ name: '情绪评分', type: 'line', smooth: true, data: rows.map(v => v.sentimentScore) }, { name: '上涨家数', type: 'line', yAxisIndex: 1, data: rows.map(v => v.risingCount) }, { name: '下跌家数', type: 'line', yAxisIndex: 1, data: rows.map(v => v.fallingCount) }] };
});
const percent = (v?: number) => v == null ? '-' : Number(v).toFixed(2) + '%';
const minutes = (v?: number) => v == null || Number(v) === 0 ? '-' : Number(v).toFixed(0) + ' 分钟';
const executionStatus = (v?: string) => ({ TRACKED: '已形成闭环', NO_RESPONSE: '暂无交易响应', NO_ALERT: '暂无预警' }[v || ''] || v || '-');
</script>

<style scoped>
.analytics{display:flex;flex-direction:column;gap:14px;margin-bottom:14px}.indicator-grid,.chart-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.indicator-grid small,.indicator-grid strong,.indicator-grid span{display:block}.indicator-grid strong{font-size:24px;margin:7px 0}.indicator-grid small,.indicator-grid span,.head span{color:#64748b}.chart-grid{grid-template-columns:repeat(2,1fr)}.head{display:flex;justify-content:space-between;gap:12px}@media(max-width:900px){.indicator-grid{grid-template-columns:repeat(2,1fr)}.chart-grid{grid-template-columns:1fr}}@media(max-width:600px){.indicator-grid{grid-template-columns:1fr 1fr}.head{flex-direction:column}}
</style>