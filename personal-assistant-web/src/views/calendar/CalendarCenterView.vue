    ...
    <section v-loading="loading" class="calendar-surface">
      ...
      <div v-else-if="view === 'MONTH'" class="month-view">
        ...
        <div class="month-grid">
          <article v-for="day in monthDays" :key="day.key" :class="['month-day', { muted: !day.current, today: day.today }]" @dblclick="openCreate(day.date)">
            ...
            <button v-for="event in day.events.slice(0, 5)" :key="event.key" class="event-chip" :style="eventStyle(event)" @click.stop="openEvent(event)"><i v-if="event.conflict" />{{ timeLabel(event) }} {{ event.title }}</button>
      ...
      <div v-else class="day-view">
        ...
        <div class="timeline">
          ...
          <button v-for="event in dayEvents" :key="event.key" class="timeline-event" :style="eventStyle(event)" @click="openEvent(event)"><time>{{ timeRange(event) }}</time><b>{{ event.title }}</b><span>{{ sourceName(event.sourceType) }} · {{ statusName(event.status) }}</span><em v-if="event.conflict">时间冲突</em></button>
        </div>
      </div>
    </section>
