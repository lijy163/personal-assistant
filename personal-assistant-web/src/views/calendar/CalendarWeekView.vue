    ...
    <div class="week-canvas">
      ...
      <div class="all-day-row">
        ...
        <div v-for="day in days" :key="day.date" class="all-day-cell" @dblclick="$emit('create', day.date, 9, 0)">
          <button v-for="event in day.topEvents.slice(0, 3)" :key="event.key" class="all-day-event" :style="baseStyle(event)" @click.stop="$emit('open-event', event)"><i v-if="event.conflict" /><span>{{ event.title }}</span><small>{{ topEventTime(event) }}</small></button>
