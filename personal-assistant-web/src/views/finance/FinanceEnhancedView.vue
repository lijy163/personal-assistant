<template>
  <div class="finance-page">
    ...
    <el-dialog v-model="accountVisible" title="新增资金账户" width="480px"><el-form :model="accountForm" label-width="90px"><el-form-item label="名称" required><el-input v-model="accountForm.accountName" maxlength="120" placeholder="例如：生活支付宝" @keyup.enter="saveAccount" /></el-form-item><el-form-item label="类型" required><el-select v-model="accountForm.accountType"><el-option label="支付宝" value="ALIPAY" /><el-option label="微信" value="WECHAT" /><el-option label="银行卡" value="BANK" /><el-option label="现金" value="CASH" /></el-select></el-form-item><el-form-item label="机构"><el-input v-model="accountForm.institution" maxlength="120" /></el-form-item><el-alert title="同一类型可以创建多个账户，但账户名称不能重复。" type="info" :closable="false" /></el-form><template #footer><el-button :disabled="accountSaving" @click="accountVisible=false">取消</el-button><el-button type="primary" :loading="accountSaving" @click="saveAccount">保存</el-button></template></el-dialog>
...
const previewData=ref<ImportPreview>(),uploading=ref(false),transactionLoading=ref(false),saving=ref(false),accountSaving=ref(false),accountVisible=ref(false),ruleVisible=ref(false),transactionVisible=ref(false),editingId=ref<number>(),transactionFormRef=ref<FormInstance>();
...
function openAccount(){Object.assign(accountForm,{accountName:'',accountType:'ALIPAY',institution:'',currency:'CNY',enabled:true});accountVisible.value=true;}
async function saveAccount(){if(accountSaving.value)return;if(!accountForm.accountName.trim())return ElMessage.warning('请输入账户名称');accountSaving.value=true;try{await saveFinanceAccount({...accountForm,accountName:accountForm.accountName.trim(),institution:accountForm.institution.trim()||undefined});ElMessage.success('资金账户已创建');accountVisible.value=false;await base();}finally{accountSaving.value=false;}}
