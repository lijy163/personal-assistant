<template>
  <div class="finance-page">
    <el-tabs v-model="tab" @tab-change="loadTab">
      ...
      <el-tab-pane label="账户与规则" name="settings">
        <el-row :gutter="16">
          ...
          <el-col :xs="24" :lg="12"><el-card><template #header><div class="toolbar"><span>分类规则</span><el-button type="primary" @click="openRule()">新增</el-button></div></template><el-table :data="rules"><el-table-column prop="ruleName" label="规则" /><el-table-column prop="keyword" label="关键词" /><el-table-column label="分类"><template #default="{row}">{{ categoryName(row.categoryId) }}</template></el-table-column><el-table-column label="操作" width="80"><template #default="{row}"><el-button link type="primary" @click="openRule(row)">编辑</el-button></template></el-table-column></el-table></el-card></el-col>
        </el-row>
      </el-tab-pane>
    </el-tabs>
    ...
    <el-dialog v-model="ruleVisible" :title="ruleEditingId?'编辑分类规则':'新增分类规则'" width="480px"><el-form :model="ruleForm" label-width="90px"><el-form-item label="名称"><el-input v-model="ruleForm.ruleName" /></el-form-item><el-form-item label="关键词"><el-input v-model="ruleForm.keyword" /></el-form-item><el-form-item label="分类"><el-select v-model="ruleForm.categoryId"><el-option v-for="item in categories" :key="item.id" :label="item.categoryName" :value="item.id" /></el-select></el-form-item></el-form><template #footer><el-button @click="ruleVisible=false">取消</el-button><el-button type="primary" @click="saveRule">保存</el-button></template></el-dialog>
  </div>
</template>
...
const textImportVisible=ref(false),textInput=ref(''),textRows=ref<FinanceTextParseRow[]>([]),ignoredLineCount=ref(0),textAccountId=ref<number>(),textParsing=ref(false),textSaving=ref(false),ruleEditingId=ref<number>();
